...
from BERT_cluster_supersenses import Bert_cluster
...
#python ./Code/test_relations_clustering.py hp1.txt
# if os.name == 'nt':
...
def extract_sentences_with_verbs(sentences):
    ...
    print('sentences with verbs',len(verified_sentences))
...
def extract_nchar_sentences(sentences, number_of_char=2):
    ...
    print('2 char sentences',len(sentences_right))
...
##    clusters = bert_cls.kmeans(k)
    ...
    #print('dict_suoersense',supersense)
       
    embedding_supersense=bert_cls.embedding_supersense()
        
    supersense_clusters =bert_cls.relate_verb_supersense()
    print(supersense_clusters)
    #
    # df1 = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in supersense_clusters.items() ]))
    # df1.to_pickle(".//supersense_cluster_average_new.pkl")
##    pd.DataFrame(supersense_clusters).T.reset_index().to_csv(".//supersense_cluster.csv", header=False, index=False)
##    pd.DataFrame.from_dict(".//supersense_cluster.csv")
    '''
    Do the embedding part for ambiguous terms.
    Need to test two approaches
    One with average
    One with the current+ average
    '''
    print('DONE SUPERSENSE CLUSTERING LEVEL 1')
    
    
    print('Start Second Phase-Hypernym based clustering')
    clusters=bert_cls.find_common_hypernyms()
    
# =============================================================================
#     print(len(hypernym_clusters),len(outliers),len( cluster_ids_hypernym),len(cluster_ids_super))
#     df2 = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in hypernym_clusters.items()]))
#     df2.to_csv("/Users/vanikanjirangat/opt/anaconda3/envs/NLP/novel2graph_sample/hypernym_clusters.pkl")
#     df3 = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in outliers.items()]))
#     df3.to_csv("/Users/vanikanjirangat/opt/anaconda3/envs/NLP/novel2graph_sample/outliers_clusters.pkl")
#     df4 = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in cluster_ids_hypernym.items()]))
#     df4.to_csv("/Users/vanikanjirangat/opt/anaconda3/envs/NLP/novel2graph_sample/cluster_ids_hypernym.pkl")
#     df5 = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in cluster_ids_super.items()]))
#     df5.to_csv("/Users/vanikanjirangat/opt/anaconda3/envs/NLP/novel2graph_sample/cluster_ids_super.pkl")
# =============================================================================
    print('DONE SUPERSENSE CLUSTERING LEVEL 2')
    


##    df = pd.read_csv(".//supersense_cluster.csv", index_col=0)
##    d = df.to_dict("split")
...
##    tsne_s = tsne(list(bert_cls.embedding.values()), list(bert_cls.embedding.keys()))
    ...
    # embed_cluster_to_dataframe(bert_cls.verbs, bert_cls.sentences_dictionary, bert_cls.all_cluster,
    #                       bert_cls.removed_chars, bert_cls.embedding,  label_verbs,
    #                       result_folder + book + '/' + book + '_relations' + str(chars_number) + '.pkl', chars_number)
##
##
...
def extract_relations(book_filename, aliases_occurrences, k=50):
    ...
    print('dealiased_book_path',dealiased_book_path )
    chars_dataframe_path = dealiased_folder + book + '_characters.pkl'
    print('chars_dataframe_path',chars_dataframe_path )
    ...
    #sentences_zero, sentences_few, sentences_right, sentences_more = extract_nchar_sentences(number_of_char=2,                                                                                sentences=all_sentences)
    #print("sentences_zero, sentences_few, sentences_right, sentences_more", len(sentences_zero),len(sentences_few),len(sentences_right),len(sentences_more))
