...
chars_embedding = pd.read_pickle('./../../Data/embedding/embeddings/hp1_dynamic.pkl')
chars_distances = pd.read_pickle('./../../Data/embedding/embeddings/hp1_dynamic_distances.pkl')


filter_chars = []
for i,name in chars_embedding['Name'].items():
    embeddings = chars_embedding.loc[chars_embedding['Name'] == name, 'Embedding'].values[0]
    for embed in embeddings:
        if embed is not None:
            filter_chars.append({'label': name, 'value': i})
            break

plot_traj = plot_trajectories(chars_embedding,
